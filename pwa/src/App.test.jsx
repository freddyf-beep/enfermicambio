...
describe('EnfermiCambio PWA shell', () => {
  ...
  it('boots in safe demo mode and renders the five original destinations', async () => {
    root = createRoot(document.getElementById('root'))
    await act(async () => { root.render(<App />); await new Promise(resolve => setTimeout(resolve, 25)) })
    expect(document.body.textContent).toContain('EnfermiCambio')
    expect(document.body.textContent).toContain('Hoy')
    for (const label of ['Hoy', 'Ranking', 'Registrar', 'Juego', 'Nosotros']) {
