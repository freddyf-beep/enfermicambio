...
function ActiveWorkout() {
  ...
  useEffect(() => {
    // Presence is handled by Supabase activity data in Enfermicambio; the
    // original openGym Node heartbeat is intentionally disabled.
    return undefined
  ...
  return <div className="narrow">
    <div className="hdr">
      <button className="iconbtn" aria-label={t('Discard')} onClick={() => confirmSheet({ title: t('Discard workout?'), message: t('The sets you logged in this session will be lost.'), confirmText: t('Discard'), danger: true, onConfirm: () => { update(s => { s.active = null }); stopRest(); nav('/train') } })}><Icon name="xmark" /></button>
