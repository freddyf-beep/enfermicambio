...
import { pushSupported, currentPushState, enablePush, disablePush, sendTestPush } from '../lib/push.js'
...
export default function Settings() {
  ...
  const doExport = async () => {
    ...
    const name = 'enfermicambio-training-' + todayISO() + '.json'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('no es una copia de EnfermiCambio')
  ...
  return <div className="narrow">
    <div className="hdr">
      <button className="iconbtn" onClick={() => nav('/train')} aria-label={t('Home')}><Icon name="chevronLeft" /></button>
      <div style={{ flex: 1, marginLeft: 10 }}><h1>{t('Settings')}</h1></div>
    </div>

    {/* ---------- account (demo and mobile builds have nothing to sign in to) ---------- */}
    <Section title="Cuenta"><Row icon="personCircle" iconTint="var(--acc)" title={user?.name || 'Atleta'} subtitle="Entrenamiento sincronizado con tu cuenta de Supabase." /></Section>
    {false && <Section title={MOBILE ? (user ? t('Your server') : t('Your data')) : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? (user ? <>
    ...
    <Section title={t('Appearance')} footer={DEMO || MOBILE ? undefined : t('synced with your profile')}>
      ...
      <div className="lrow ec-icon-setting">
        <div className="grow"><span className="lrow-t">Icono al instalar</span><span className="lrow-s">Elígelo antes de añadir EnfermiCambio a la pantalla de inicio.</span></div>
        <div className="ec-icon-options" aria-label="Icono de instalación">
          {INSTALL_ICONS.map(icon => <button key={icon.id} className={(S.installIcon || 'pulse') === icon.id ? 'on' : ''} onClick={() => update(s => { s.installIcon = icon.id })} aria-label={icon.label} aria-pressed={(S.installIcon || 'pulse') === icon.id}>
            <img src={`icon-${icon.id}-180.png`} alt="" />
          </button>)}
        </div>
      </div>
    </Section>
    ...
    <Section title={t('Data')}>
      ...
      <Row icon="trash" iconTint="var(--red)" title={t('Reset everything')} danger onClick={() => confirmSheet({ title: t('Reset everything?'), message: t('Deletes your plan, workouts and body weight on this device. This cannot be undone.'), confirmText: t('Delete everything'), danger: true, onConfirm: () => { replaceState(JSON.parse(JSON.stringify(DEF)), true); nav('/train'); toast(t('All data reset')) } })} />
    </Section>
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={'Instala EnfermiCambio a pantalla completa. ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}
    ...
    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      EnfermiCambio v{__APP_VERSION__}<br />
      Motor de entrenamiento adaptado de <a href="https://gitlab.com/DuarteSantos8/opengym" target="_blank" rel="noopener">OpenGym</a> (AGPL v3) · ejercicios: hasaneyldrm/exercises-dataset (MIT)<br />
      imágenes y animaciones © <a href="https://gymvisual.com/" target="_blank" rel="noopener">Gym visual</a>
    </div>
  </div>
}
...
function PushCard({ S, update, toast }) {
  ...
  const on = pushState === 'enabled'

  const refresh = async () => {
    try { setPushState((await currentPushState()).code) }
  ...
  const test = async () => {
    try { await sendTestPush(); toast('Prueba solicitada; puede tardar unos segundos.') }
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={pushState === 'desynced' ? 'El permiso existe, pero este dispositivo debe reconectarse.' : 'Avisos importantes incluso con EnfermiCambio cerrada.'}>
...
function PairSheet({ close }) {
  ...
  return <>
    ...
    <div className="muted small" style={{ marginBottom: 14 }}>
      En EnfermiCambio, elige “Conectar a mi servidor” e introduce esta dirección y el código. Caduca en 5 minutos.
    </div>
