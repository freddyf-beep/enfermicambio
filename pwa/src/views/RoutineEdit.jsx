...
import { activeProfile, exAvailable, homeReplacementFor } from '../lib/equipment.js'
...
export default function RoutineEdit() {
  ...
  const adaptToHome = () => edit(entries => {
    entries.forEach((entry, index) => {
      const original = exOr(entry.id)
      if (exAvailable(S, original)) return
      const replacement = homeReplacementFor(original)
      if (!replacement) return
      entries[index] = { ...entry, id: replacement.id, note: entry.note || `Reemplazo en casa de ${exerciseNameFor(original)}` }
    })
  })
  ...
  return <div className="narrow">
    ...
    {missingCount > 0 && <div className="card" style={{ marginBottom: 16, borderColor: 'var(--orange)' }}>
      ...
      {S.trainingPlace === 'home' && <Button size="sm" variant="tinted" icon="house" onClick={adaptToHome}>Adaptar rutina a Casa</Button>}
    </div>}
    ...
    {r.ex.length ? <div className="list">{r.ex.map((e, i) => {
      ...
      const homeReplacement = noEquip && S.trainingPlace === 'home' ? homeReplacementFor(ex) : null
      ...
      return <div key={i}>
        ...
        <div className={'item' + (inSS.has(i) ? ' in-ss' : '')} onClick={() => {
          ...
          <div className="grow"><div className="tt capitalize">{exerciseNameFor(ex)}</div><div className="ss">{exLine(e, S.unit)}</div>
            {homeReplacement && <div className="small" style={{ marginTop: 2, color: 'var(--green)' }}>En casa: {exerciseNameFor(homeReplacement)}</div>}
