...
import { loadTrainingState, saveTrainingState } from '../lib/training-cloud.js'

const KEY = 'gym_state_v1'
export const DEF = {
  unit: 'kg', restSec: 90, restPauseSec: 15, sound: true, keepAwake: true, lang: 'es',
  theme: 'dark', accent: 'lime', installIcon: 'pulse', body: 'male', targetW: null,
  ...
  equipProfiles: [], activeEquipId: null, equipFilterOn: false, trainingPlace: null,
...
export const useStore = create((set, get) => {
  ...
  return {
    ...
    async pushState() {
      if (!get().user) return
      clearTimeout(pushTm)
      try { await saveTrainingState(get().S); localStorage.removeItem('gym_dirty') }
    ...
    async signOut() {
      try { await get().pushState() } catch (e) { /* local data remains */ }
      clearLocalSession()
    },
    ...
    async boot() {
      // Enfermicambio uses Supabase for identity and shared data. The imported
      // training engine remains local-first and never calls openGym's Node API.
      if (!MOBILE && !DEMO) {
        get().setGuest(true)
        set({ ready: true })
        return
