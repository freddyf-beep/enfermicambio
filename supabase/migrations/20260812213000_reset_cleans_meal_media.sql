-- Extend the private reset to remove meal-photo blobs as well as their
-- database references. Other buckets (avatars/workout media) are preserved.
...
begin
  ...
  result := result || jsonb_build_object('meal_media_objects', deleted_count);
