...
- [x] 3.2 Schedule `close_round` at 12:00, 18:00, 00:00 in `competition_timezone` via Supabase scheduled jobs. Each job writes its winner row once; retries are idempotent via uniqueness constraints. Cron jobs `close-round-morning/afternoon/night` active via pg_cron + pg_net calling the deployed function (2026-08-11).
- [ ] 3.3 Implement point rules from `app_config`: daily rank (10/7/4/2), round wins (+3 each), step goal (+2), workout (+3), within calorie target (+2), mission rewards (variable). Mission rewards (variable) and achievement points now awarded by `evaluate_missions`/`evaluate_achievements` from `close_day` (2026-08-11). Workout (+3) and calorie-target (+2) bonuses pending.
...
- [x] 3.6 Implement `close_day`: closes night round, awards daily rank points, evaluates daily streaks, evaluates achievements, updates historical stats, publishes the daily summary event, all inside one transaction. `close_day` Edge Function deployed; awards daily rank points (10/7/4/2), updates step-goal streaks idempotently, evaluates the day's rotating missions (`evaluate_missions`) and lifetime achievements for all four users (`evaluate_achievements`) (2026-08-11). Historical stats/transactionality pending refinement.
...
- [x] 3.8 Implement the generic achievement engine: `metric`, `operator`, `threshold`, `time_window`, `repeatable`, `hidden`, `season_points`. Seed the initial pack from `SPECS.md` section 65. Non-repeatable achievements get a uniqueness constraint on `(user_id, achievement_id)`. `AchievementEngine` + tests; uniqueness constraint in migration 0002; 15 achievements + 10 missions seeded to remote DB in migration `20260811002255` (2026-08-11). Seeds translated to Spanish and 4 km achievements added in migration `20260811120100` (2026-08-11).
- [x] 3.9 Implement mission engine for individual, competitive, and cooperative missions. Seed the initial pack from `SPECS.md` section 66. Cooperative progress uses a group row (`user_id` null). `MissionEngine` + 4 tests; 10 missions seeded to remote DB in migration `20260811002255` (2026-08-11). Daily rotation (2 individual + 1 cooperative + 1 competitive, deterministic per date) via `daily_missions_for_date`; daily progress via `mission_progress.progress_date` (2026-08-11).
- [x] 3.10 Implement `close_season`: freeze standings, write `season_results` for all four users, award champion trophy, publish season result event, create next season. Transactional; safe to retry. `close_season` Edge Function deployed; verified closing an ended season, writing results, and creating the next season (2026-08-11). Champion trophy post included; transactionality is best-effort across statements.
- [x] 3.11 Build the `JUEGO` screen: current season, standings, today's missions, streaks, achievements, trophy cabinet, season history. Implemented with live data: rotating daily missions with progress, streaks, achievement grid (unlocked/locked/secret), battle pass track with claims, season km, standings, and season history with champions (2026-08-11). See `docs/checkpoint-gamificacion.md`.
...
- [x] 4.9 Push notifications via FCM/APNs with per-user preference categories: overtakes, round endings, achievements, workouts, comments/reactions, missions, season results. Copy tone per `SPECS.md` section 68: playful, never medical or shame-oriented. Server-authoritative `notifications` table + social triggers + `generate_events` Edge Function (overtakes, leader change, milestones, records, daily goal) + round/day/season notifications in the closing functions + in-app center (bell with badge, list, mark read, preferences toggles) + `flutter_local_notifications` delivery when app is backgrounded, FCM-ready interface. Migrations pushed and functions deployed to `bweynxdzovnbcjwgddar` (2026-08-11). FCM channel itself remains future work.
...
- [x] 6.10 Season history with champions list. Implemented in JUEGO (`Historial de Temporadas`): seasons closed with champion name per season from `season_results` (2026-08-11).
...
| 4 | 2026-08-11 | In progress | Feed with pagination + card types; post composer; reactions + comments; RLS blocks forged system posts (403 verified); Realtime wired for posts (refetch from DB); leader-change events rate-limited; notifications center + server events + preferences + weight log (see row 9) | Media pipeline, system events (4.6 partial), FCM push channel pending |
...
| 8 | 2026-08-11 | Completed | 100% Spanish transformation completed. Premium Strava/Apple Fitness theme added. All 5 tabs (HOY, RANKING, REGISTRAR, JUEGO, NOSOTROS) redesigned. Login with 1-tap quick fill for the 4 real users (Freddy, Felipe, Cristian, Samir) with password '123456'. Mock suite created in test/mocks/user_mocks.dart. All tests updated and passing. | Local build complete; GitHub push & IPA build bypassed per user local policy. |
| 9 | 2026-08-11 | In progress | Notifications system (4.9): `notifications` table + RLS, social triggers (post/comment/reaction), `generate_events` Edge Function, round/day/season notifications, `insert_notification`/`notify_weight_goal`/`notify_round_ending` functions, cron reminders at 11:30/17:30/23:30, Realtime channel; in-app bell + list + preferences; local system notifications (FCM-ready); private weight log (table, goal, screen, weekly/goal notifications). Migrations + 4 functions deployed; `flutter analyze` clean; 63 tests passing. | FCM push channel pending (interface ready); smoke test script `supabase/scripts/notifications_smoke_test.sql` not yet executed against remote; weight "import" source unused |
| 10 | 2026-08-11 | In progress | Full gamification (3.3/3.9/3.11/6.10): daily rotating missions with real progress, achievement evaluation in `close_day`, 19 achievements + 13 missions in Spanish incl. km content, battle pass (10 tiers, claims, titles, exclusive emojis), season km, season history with champions, in-app celebrations via Realtime, RANKING with real per-category per-period data. 3 migrations ready for `supabase db push` + `close_day` extended (bundle checked); `flutter analyze` 0 errors; 64 tests passing. See `docs/checkpoint-gamificacion.md`. | Migrations not yet applied to remote (CLI unavailable on this host); E2E day simulation (3.13) pending; battle pass balance tuning in seed |
